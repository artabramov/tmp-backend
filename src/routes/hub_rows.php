...
$query->text = "SELECT hubs.*, user_roles.user_role FROM hubs LEFT JOIN user_roles ON hubs.id=user_roles.hub_id WHERE hubs.id IN (SELECT user_roles.hub_id FROM user_roles WHERE user_roles.user_id = ?)  ORDER BY hubs.id DESC LIMIT " . HUBS_SELECT_LIMIT . " OFFSET " . $offset;
$query->args = [ $self_user->id ];
