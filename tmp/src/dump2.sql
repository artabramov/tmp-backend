...
CREATE TABLE IF NOT EXISTS hubs (
    ...
    roles_count BIGINT(20) UNSIGNED NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS user_roles (
    ...
    user_role  ENUM('admin', 'author', 'editor', 'reader', 'invited', 'banned') NOT NULL,
...
FOR EACH ROW 
BEGIN
    SET @roles_count := (SELECT COUNT(id) FROM user_roles WHERE hub_id = NEW.hub_id AND user_role NOT IN ('invited', 'banned'));
...
FOR EACH ROW 
BEGIN
    SET @roles_count := (SELECT COUNT(id) FROM user_roles WHERE hub_id = OLD.hub_id AND user_role NOT IN ('invited', 'banned'));
...
FOR EACH ROW 
BEGIN
    SET @posts_count := (SELECT COUNT(id) FROM posts WHERE hub_id = NEW.hub_id AND post_status <> 'trash');
...
FOR EACH ROW 
BEGIN
    SET @posts_count := (SELECT COUNT(id) FROM posts WHERE hub_id = OLD.hub_id AND post_status <> 'trash');
...
SELECT * FROM users; SELECT * FROM hubs; SELECT * FROM user_roles; SELECT * FROM user_pals; SELECT * FROM posts; SELECT * FROM post_comments; SELECT * FROM comment_uploads; SELECT * FROM meta; 
